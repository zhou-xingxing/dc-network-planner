...
- 做前端改动时，默认至少跑 `cd frontend && npm run lint && npm run type-check && npm run build`。
...
- 同一业务请求链路中应避免重复查询同一业务对象；上游已经获取并校验过的实体或上下文可显式传递给下游方法。但也不要为减少查询而传递过宽的上下文对象，方法签名仍应清晰表达其校验责任和业务边界。
